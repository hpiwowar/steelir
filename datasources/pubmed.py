...
@TimedCache(timeout_in_seconds=60*60*24*7)
def filter_pmids_not(query_pmids, pubmed_filter_string):
    if VERBOSE:
        print "filter_pmids for %s" %query_pmids
    ...
    try:
        (client, the_post) = get_history_client_post(query_pmids)
        filtered_pmid_records = client.search(
                    '#%s NOT (%s)' %(the_post.query_key, pubmed_filter_string), 
                    db="pubmed") 
        print len(filtered_pmid_records)
        filtered_pmids = filtered_pmid_records.dbids.ids
    except AttributeError:
...
@fieldname("pubmed_in_pmc")    
def in_pmc(query_pmids):
    matching_pattern = 'pubmed_pmc[filter]'
    flag_pmid_passes_filter = _get_flags_for_pattern(query_pmids, matching_pattern)
    return(flag_pmid_passes_filter)
...
@fieldname("pubmed_title_and_abstract")
def title_and_abstract(pmids):
    titles = article_title(pmids)
    abstracts = abstract(pmids)
    response = [mytitle + "\n" + myabstract for (mytitle, myabstract) in zip(titles, abstracts)]
    return(response)
